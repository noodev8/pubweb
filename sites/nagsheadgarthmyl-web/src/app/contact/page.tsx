          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
            ...
            <div>
              ...
              <address className="not-italic text-stone-600 space-y-1">
                <p className="font-medium text-stone-900">{venue.name}</p>
                <p>{venue.address.line1}</p>
                {venue.address.line2 && <p>{venue.address.line2}</p>}
                <p>{venue.address.town}</p>
                {venue.address.county && <p>{venue.address.county}</p>}
              ...
              <div className="mt-6 space-y-3">
                ...
                {venue.contact.bookingEmail && (
                  <p>
                    <span className="text-stone-600">Reservations: </span>
                    <a
                      href={`mailto:${venue.contact.bookingEmail}`}
                      className="text-amber-600 hover:text-amber-700 transition-colors"
                    >
                      {venue.contact.bookingEmail}
                    </a>
              ...
              <div className="mt-6 flex gap-2">
                {venue.social.facebook && (
                  <a
                    ...
                    className="p-2 -m-2 text-stone-400 hover:text-amber-600 transition-colors"
                    aria-label="Facebook"
                  >
                    <svg
                      className="h-6 w-6"
              ...
              {hours.specialClosures && hours.specialClosures.length > 0 && (
                <div className="mt-6">
                  ...
                  <ul className="text-sm text-stone-600 space-y-1">
                    {hours.specialClosures.map((closure, index) => (
                      <li key={index}>
                        {closure.reason || closure.date}
                        {closure.isClosed ? ' - Closed' : ''}
                      </li>
                    ))}
                  </ul>
                </div>
